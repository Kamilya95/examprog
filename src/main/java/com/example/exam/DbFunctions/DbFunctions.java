...
public class DbFunctions {
    ...
    public void createUser(String surname, String name, String lastname,
        ...
        try {
            String query = String.format("insert into users (фамилия, имя," +
    ...
    public int signIn(String login, String password) {
        try {
            String query = String.format("select * from users where логин = '%s' and пароль='%s'", login, password);
            Statement statement = connect_to_db().createStatement();
    ...
    public int check_login(String login) {
        try {
            String query = String.format("select * from users where логин = '%s'", login);
        ...
        try{
            ...
            while (resultSet.next()){
                users.add(new User(
                        ...
                        resultSet.getString("логин"),
                        resultSet.getString("пароль")
    ...
    public ObservableList<Service> getAllService(){
        ObservableList<Service> users = FXCollections.observableArrayList();
        try{
            ResultSet resultSet = connect_to_db().createStatement().executeQuery("select * from services");
            while (resultSet.next()){
                users.add(new Service(
                        resultSet.getString("id"),
